...
# Models
from LRNet_MAX import LRNet_MAX
...
from TOOL import *
from Z_Parameters4LRNet_MAX import Parameters
...
OUTPUT_RESULT_DIR = f"../../../../ResultRepo/{dict_dataset[TYPE_DATASET]}/LRNet_MAX"
...
# NETWORK
net, net_name = LRNet_MAX(), 'LRNet_MAX'
# print(net)

net.cuda()
criterion = HybridCDLoss(label_smoothing_para_beta=0.05, hard_ratio_para_theta=0.5)
...
def train(n_epochs=N_EPOCHS, save=True):
    ...
    for epoch_index in tqdm(range(n_epochs)):
        ...
        if save:
            ...
            plt.figure(num=3)
            plt.savefig(OUTPUT_RESULT_DIR + "/" + net_name + '-DATA' + str(TYPE_DATASET) + f'-{CUR_DATE}-accuracy-per-class.' + im_format)
            plt.figure(num=4)
...
if LOAD_TRAINED == False:
    ...
    if VAL_OR_NOT == True:
        parameters["N_VAL"]=valid_dataset.n_imgs
