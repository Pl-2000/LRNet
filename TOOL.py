...
class ChangeDetectionDataset(Dataset):
    """Change Detection Dataset (For Test) class, used for both training and test data."""
    ...
    def get_img(self, im_name):
        img1, img2, cm, cm32 = read_img_and_cm(TYPE_DATASET=self.type_dataset, path=self.path, img_name=im_name, mode_file=self.train_val_test, NORMALISE=self.normalise, mode_NORMALISE=self.MODE_NORMALISE, MEAN_STD_T1=self.MEAN_STD_T1, MEAN_STD_T2=self.MEAN_STD_T2)
        img1 = reshape_for_torch(img1)
        img2 = reshape_for_torch(img2)
        return img1, img2, cm, cm32

    def __len__(self):
        return self.n_imgs

    def __getitem__(self, idx):
        img1, img2, cm, cm32 = read_img_and_cm(TYPE_DATASET=self.type_dataset, path=self.path, img_name=self.names[idx], mode_file=self.train_val_test, NORMALISE=self.normalise, mode_NORMALISE=self.MODE_NORMALISE, MEAN_STD_T1=self.MEAN_STD_T1, MEAN_STD_T2=self.MEAN_STD_T2)
        ...
        sample = {'I1': I1, 'I2': I2, 'label': label, 'label32': label32}
